  ...
  <div class="group cursor-pointer w-full relative overflow-hidden">
    ...
    <div class="relative w-full aspect-[16/9] mb-4 overflow-hidden bg-background border-2 border-text">
      ...
      <div class="absolute inset-0 bg-primary/90 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-4">
        ...
        <!-- Project Description -->
        <p class="text-center text-sm px-6 max-w-xs text-background">
          {{ project.description }}
        </p>
        ...
        <div class="flex gap-3">
          ...
          <a 
            ...
            :href="project.github" 
            target="_blank"
            class="group/btn relative cursor-hover flex items-center gap-2 bg-background border-2 border-background px-4 py-2 text-primary hover:border-background hover:bg-transparent hover:text-background transition-all duration-300 text-sm uppercase font-bold tracking-wider"
            @click.stop
          >
          ...
          <a 
            ...
            class="group/btn relative cursor-hover flex items-center gap-2 bg-transparent border-2 border-background px-4 py-2 text-background hover:bg-background hover:text-primary transition-all duration-300 text-sm uppercase font-bold tracking-wider"
            @click.stop
          >
            <svg v-if="project.website.startsWith('mailto:')" class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              <path d="M20,8L12,13L4,8V6L12,11L20,6M20,4H4C2.89,4 2,4.89 2,6V18A2,2 0 0,0 4,20H20A2,2 0 0,0 22,18V6C22,4.89 21.1,4 20,4Z" />
            </svg>
            <svg v-else class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              <path d="M14,3V5H17.59L7.76,14.83L9.17,16.24L19,6.41V10H21V3M19,19H5V5H12V3H5C3.89,3 3,3.9 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V12H19V19Z" />
            </svg>
            {{ project.website.startsWith('mailto:') ? 'Contact' : 'Visit' }}
          </a>
          
          <!-- Mobile App Badge (when no website) -->
          <span 
            v-else-if="project.type === 'Mobile Development'"
            class="flex items-center gap-2 bg-background/20 border-2 border-background/30 px-4 py-2 text-background text-sm uppercase font-bold tracking-wider cursor-not-allowed"
          >
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
